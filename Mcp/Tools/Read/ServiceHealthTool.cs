...
/// <summary>Reachability of one upstream service.</summary>
public sealed record ServiceHealthEntry(string Service, string Url, bool Reachable, int? StatusCode, long LatencyMs, string? Error);
...
public sealed class GetServiceHealthTool : JsonReadTool<EmptyArgs, GetServiceHealthTool.Output>
{
    ...
    public override ToolDescriptor Descriptor { get; } = ToolDescriptor.WithStaticSchema(
        ...
        "depend on it (e.g. before search_observations or launch_session).",
    ...
    protected override async Task<Output> HandleAsync(EmptyArgs args, McpToolContext context, CancellationToken ct)
    {
        ...
        return new Output(services.Count, services.Count(s => s.Reachable), services);
    }

    public sealed record Output(int Count, int ReachableCount, IReadOnlyList<ServiceHealthEntry> Services);
}
