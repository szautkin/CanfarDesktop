...
/// and the in-progress edit state for the inline editor. WinUI-free so the logic is unit-testable.
/// </summary>
public partial class AiGuideToolRowViewModel : ObservableObject
{
    ...
    public AiGuideToolRowViewModel(string name, string defaultDescription, string effectiveDescription, bool isOverridden, string categoryId)
    {
        Name = name;
...
/// <summary>A category card: its metadata, its tool rows, and filter/override-count state.</summary>
...
/// <summary>
/// Backs the AI Guide dashboard: projects the live agent-tool surface into editable category cards
/// (merging stored description overrides) and lists the user's guide tools. Pure logic over
/// <see cref="AiGuideService"/> + a tool-input provider, so it can be unit-tested without WinUI.
/// </summary>
public partial class AiGuideViewModel : ObservableObject
{
    ...
    public string StatsText => $"{ToolCount} tools · {OverriddenCount} overridden · {CategoryCount} categories";
    ...
    public void Load()
    {
        ...
        foreach (var cat in AiGuideCatalog.AllCategories) // ordered; Other last
        {
            ...
            foreach (var r in catRows)
                group.Tools.Add(new AiGuideToolRowViewModel(r.Name, r.DefaultDescription, r.EffectiveDescription, r.IsOverridden, cat.Id));
