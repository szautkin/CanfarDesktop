...
public class ImageDiscoveryCoordinator
{
    ...
    private async Task<ImageManifest> RunDiscoveryAsync(string imageID, bool force, CancellationToken ct)
    {
        ...
        catch (ImageDiscoveryException ide) { PersistFailure(imageID, ide, null); throw; }
        catch (HeadlessLaunchException hle) { var e = ImageDiscoveryException.JobSubmitFailed(hle.Message); PersistFailure(imageID, e, null); throw e; }
        catch (Exception ex) { var e = ImageDiscoveryException.JobSubmitFailed(ex.Message); PersistFailure(imageID, e, null); throw e; }
        ...
        catch (Exception ex) { var e = ImageDiscoveryException.ManifestFetchFailed(ex.Message); PersistFailure(imageID, e, jobId); throw e; }
        ...
        catch (ManifestParseException pe) { var e = ImageDiscoveryException.ManifestParseFailed(pe.Message); PersistFailure(imageID, e, jobId); throw e; }
    ...
    private async Task PollUntilTerminalAsync(string jobId, CancellationToken ct)
    {
        for (var i = 0; i < _maxPolls; i++)
        {
            ...
            if (job is null) return; // dropped from listing → reaped after completion; manifest fetch validates
            if (job.IsTerminal)
            {
