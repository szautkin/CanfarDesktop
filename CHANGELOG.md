...
- **Approval of an assistant's changes** — `start_background_apply` would start any pending change an assistant asked it to, including a destructive one waiting for your approval (removing a file, stopping a session, deleting from storage) and, with auto-apply off, any change at all. It now starts only what auto-apply would have applied without you, and says why it will not otherwise: a destructive change always waits for you
- **Marks after reopening a file** — marks were kept under the path exactly as it was written, so the same file reopened as `C:/Users/…/m31.fits` rather than `C:\Users\…\m31.fits`, or with other capitals, or with a `..\` in its path, came back with none, and `list_fits_annotations` said there were none. The path is now read one way, however it is written, and marks already kept under several spellings of one file's path are put together
- **Go To in the FITS viewer** — after a Go To off the image, one that found its position left "Coordinate outside image bounds" in the status bar; it now says where it centred. And the RA and Dec boxes read a position as the rest of the app does — degrees with a point or a comma, or sexagesimal — where a French Windows refused "10.68"
