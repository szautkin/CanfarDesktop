...
- **Cutouts from a file you have downloaded** — once an observation's file is on your computer, cut pieces of it there: at once, offline, as often as you like, and for files CADC will not cut at all, such as its HST mirror's. The same **Cutout…** button and the same editor: when both ways are open to a file, **Cut by** chooses between CADC (only the part is downloaded) and this computer (cut from the downloaded file), this computer first; the region, the checks and Research's record are the same either way, and the size shown for a local cut is exact. The pixels are copied, not converted — same BITPIX and scaling — and the header is the file's own with its size, every reference pixel, pixel sections and checksums put right for the cut and a HISTORY line saying where it came from; SIP distortion stays valid. A multi-extension file keeps its shape: an HST frame's SCI, ERR and DQ of the chips the region falls on, a MegaPrime frame's CCDs, each cut by its own sky coordinates, the others named in the primary header as left out; a region in the gap between chips is refused as on none of them. A file that cannot be cut says why — no sky coordinates, only approximate ones, galactic or B1950, tile-compressed, cut short. Research keeps the cutout as a cutout of the observation, cut on this computer; **Remove file** then **Cut out again** makes it anew from the downloaded file. Download on an observation now fetches the same file it holds, not whichever DataLink lists first. For assistants: `cutBy` (`soda` or `local`) on `download_cutout` and `show_cutout_editor`, and `get_cutout_options` lists each way a file can be cut and why one cannot
