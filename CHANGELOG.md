...
- **Cutouts from the archive** — download only part of a file, cut on CADC's side by its SODA service: a few MB of a 1.6 GB MegaPipe tile, at full resolution. In an observation's Files tab, a file CADC can cut has a **Cutout…** button that opens an editor: a circle or box by RA, Dec and size (degrees or sexagesimal, ″ ′ °), or drawn on the file's footprint — drag to move, drag its dot to resize, drag elsewhere to draw — and, for a cube, a wavelength range. It starts from your last search's target and wavelengths when they fall on the file, checks as you type (off the file, partly off, a band the file does not have), and estimates the size. The cutout downloads like any download — in the status bar, carrying on if you move away — and is kept in Research as the observation's record, marked **Cutout** with the region, and opening with a note that it is not the complete observation. A mark on an image opened from Research has **Cut out from the archive…** in its menu. Search's **Spatial cutout** and **Spectral cutout** boxes now do what they say: a row's download is cut to the search's circle and wavelengths. For assistants: `get_cutout_options`, `download_cutout` (checked before it is queued, with the reason when it is refused) and `show_cutout_editor`
